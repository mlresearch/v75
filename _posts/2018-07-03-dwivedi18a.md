...
abstract: We consider the problem of sampling from a strongly log-concave density
  in $\mathbbR^d$, and prove a non-asymptotic upper bound on the mixing time of the
  Metropolis-adjusted Langevin algorithm (MALA). The method draws samples by running
  ...
  $\mathcalO \big(κd \log(1/δ) \big)$ steps, as compared to the $\mathcalO \big(κ^2
  ...
  it mixes $\mathcalO(κd)$ slower than MALA.
